...
import { FiPlus, FiEdit2, FiTrash2, FiX, FiSave, FiFilter, FiSearch, FiExternalLink, FiCheckSquare, FiEye, FiRotateCcw, FiHome, FiMapPin, FiCalendar, FiCheckCircle, FiCamera, FiUpload } from 'react-icons/fi';
...
const Jobs = () => {
    ...
    // OTP & Settings State (For Jobs Page only needed if used in other modals?? 
    // OTP was only for New Order form. Payment modal doesn't use OTP usually.
    // So remove OTP state too?)
    ...
    const paymentCameraRef = useRef(null);
    const returnFileRef = useRef(null);
    const returnCameraRef = useRef(null);

    // Camera Modal Logic
    const [showCamera, setShowCamera] = useState(false);
    const [cameraTarget, setCameraTarget] = useState(null); // 'before', 'payment', 'return'
    ...
    const handleCameraCapture = (file) => {
        ...
        if (cameraTarget === 'payment' || cameraTarget === 'return') {
    ...
    const handleEdit = (job) => {
        navigate(`/jobs/edit/${job.jobId || job.id || job._id}`);
    };
                                ...
                                <tr key={job._id || job.id || job.jobId} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <div className="flex flex-col gap-1 items-start">
                                            ...
                                            {job.isWarranty && (
                                                <span className="flex items-center gap-1 text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded border border-blue-200 font-semibold whitespace-nowrap">
                                                    <FiCheckCircle className="w-3 h-3" /> Warranty
                                                </span>
                                            )}
                                        </div>
                ...
                <form onSubmit={submitPayment} className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mb-2">
                            <button type="button" onClick={() => paymentFileRef.current.click()} className="flex-1 flex items-center justify-center gap-2 py-2 px-3 border border-gray-300 rounded-lg bg-white text-gray-700 hover:bg-gray-50 text-sm transition-all">
                                <FiUpload /> Gallery
                            </button>
                            <button type="button" onClick={() => openCamera('payment')} className="flex-1 flex items-center justify-center gap-2 py-2 px-3 border border-emerald-200 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 text-sm transition-all">
                                <FiCamera /> Camera
                            </button>
                        </div>
                        <input type="file" ref={paymentFileRef} multiple accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, setPaymentFiles)} />
                        <input type="file" ref={paymentCameraRef} accept="image/*" capture="environment" className="hidden" onChange={(e) => handleImageUpload(e, setPaymentFiles)} />
            ...
            >
                <form onSubmit={submitReturn} className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mb-2">
                            <button type="button" onClick={() => returnFileRef.current.click()} className="flex-1 flex items-center justify-center gap-2 py-2 px-3 border border-gray-300 rounded-lg bg-white text-gray-700 hover:bg-gray-50 text-sm transition-all">
                                <FiUpload /> Gallery
                            </button>
                            <button type="button" onClick={() => openCamera('return')} className="flex-1 flex items-center justify-center gap-2 py-2 px-3 border border-orange-200 rounded-lg bg-orange-50 text-orange-700 hover:bg-orange-100 text-sm transition-all">
                                <FiCamera /> Camera
                            </button>
                        </div>
                        <input type="file" ref={returnFileRef} multiple accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, setPaymentFiles)} />
                        <input type="file" ref={returnCameraRef} accept="image/*" capture="environment" className="hidden" onChange={(e) => handleImageUpload(e, setPaymentFiles)} />
                        <p className="text-xs text-gray-500 mt-1">{paymentFiles.length} files selected</p>
                        {paymentFiles.length > 0 && (
                            <div className="mt-2 grid grid-cols-4 gap-2">
                                {paymentFiles.map((file, index) => (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between items-start bg-gray-50 p-4 rounded-xl border border-gray-100">
                            ...
                            <div className="flex flex-col items-end gap-1">
                                ...
                                {viewJob.isWarranty && (
                                    <span className="flex items-center gap-1 text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full border border-blue-200 font-bold whitespace-nowrap">
                                        <FiCheckCircle className="w-3 h-3" /> Warranty
                                    </span>
                                )}
                            </div>
                                ...
                                <div className="card p-4 border border-gray-100 shadow-none">
                                    ...
                                    <div className="space-y-2 text-sm">
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className={`font-medium ${viewJob.isWarranty ? 'text-blue-600 font-bold flex items-center gap-1' : 'text-gray-800'}`}>
                                                {viewJob.isWarranty && <FiCheckCircle className="w-4 h-4" />}
                                                {viewJob.isWarranty
                                                    ? (viewJob.warranty || 'Covered Under Warranty')
                                                    : (viewJob.warranty || 'No Warranty')}
                                            </span>
                                        </div>
